    ...
    repeat with w in windows
      ...
      if windowName contains "localhost" or windowName contains "The Final Burger" or windowName contains "8080" or windowName contains "8081" then
        ...
        if windowCount is 1 then
          set bounds of w to {0, 38, 502, 893}
          log "Ventana 1 reposicionada a {0, 38, 502, 893}"
        else if windowCount is 2 then
          set bounds of w to {350, 38, 850, 893}
          log "Ventana 2 reposicionada a {350, 38, 850, 893}"
        end if
      end if
    end repeat
    return windowCount
  ...
  osascript -e 'tell application "System Events"
    tell process "Cursor"
      tell window 1
        ...
        set size to {661, 855}
        log "Ventana Cursor reposicionada a posición {851, 38} y tamaño {661, 855}"
      end tell
    end tell
...
# Ejecutar reposicionamiento usando script de Python
...
# Esperar un poco más y hacer un segundo intento
sleep 5
echo -e "\n${BLUE}🔄 Segundo intento de reposicionamiento...${NC}"
python3 ./scripts/run/reposition_flutter_windows.py
